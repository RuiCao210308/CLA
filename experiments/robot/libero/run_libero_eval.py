    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
                ...
                try:
                    ...
                    if cfg.action_correction.enabled:
                        log_file.write(
                            ...
                            f"candidate_triggered={correction_debug_state['candidate_triggered']}, "
                            f"severity_score={correction_debug_state['severity_score']:.6f}, "
                            f"fatigue_score={correction_debug_state['fatigue_score']:.6f}, "
                            f"in_grasp_window={correction_debug_state['in_grasp_window']}, "
                            f"closing_attempt={correction_debug_state['last_closing_attempt']}, "
                            f"grasp_window_smoothing_blocked={correction_debug_state['last_grasp_window_smoothing_blocked']}, "
                            f"grasp_window_close_delayed={correction_debug_state['last_grasp_window_close_delayed']}, "
                            f"remaining_close_delay_budget={correction_debug_state['remaining_close_delay_budget']}, "
                            f"close_attempt_xy_norm={correction_debug_state['close_attempt_xy_norm']:.6f}, "
                            f"close_attempt_xyz_norm={correction_debug_state['close_attempt_xyz_norm']:.6f}, "
            ...
            print(
                ...
                f"correction_steps_used={int(correction_metrics['correction_steps_used'])}, "
                f"correction_ratio={correction_metrics['correction_ratio']:.6f}, "
                f"candidate_trigger_count={int(correction_metrics['candidate_trigger_count'])}, "
                f"mean_severity_on_candidates={correction_metrics['mean_severity_on_candidates']:.6f}, "
                f"mean_fatigue_on_candidates={correction_metrics['mean_fatigue_on_candidates']:.6f}, "
                f"severity_score={correction_metrics['severity_score']:.6f}, "
                f"fatigue_score={correction_metrics['fatigue_score']:.6f}, "
                f"grasp_window_trigger_count={int(correction_metrics['grasp_window_trigger_count'])}, "
                f"grasp_window_steps={int(correction_metrics['grasp_window_steps'])}, "
                f"grasp_window_active_ratio={correction_metrics['grasp_window_active_ratio']:.6f}, "
                f"grasp_window_close_delayed_count={int(correction_metrics['grasp_window_close_delayed_count'])}, "
                f"grasp_window_smoothing_block_count={int(correction_metrics['grasp_window_smoothing_block_count'])}, "
                f"close_attempt_count={int(correction_metrics['close_attempt_count'])}, "
                f"delayed_close_ratio={correction_metrics['delayed_close_ratio']:.6f}, "
                f"mean_close_attempt_xy_norm={correction_metrics['mean_close_attempt_xy_norm']:.6f}, "
                f"mean_close_attempt_xyz_norm={correction_metrics['mean_close_attempt_xyz_norm']:.6f}, "
